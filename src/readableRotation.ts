...
export function readableRotationText(
  ...
  const breakRows = new Set<string>(
    skillRows.filter(row => row.step.type === "skill" && row.step.causesBreak).map(row => row.id),
  )
  const breakEvents = timeline.filter(
    row =>
      row.kind === "rotation" &&
      row.step.type === "event" &&
      row.step.event === "Qi" &&
  ...
  breakEvents.forEach(breakEvent => {
    const containingRow =
      skillRows.find(row => row.id === breakEvent.sourceRowId) ??
      skillRows.find(
        row =>
          breakEvent.startTime >= row.startTime - 1e-6 &&
          breakEvent.startTime <= row.startTime + row.effectiveCastTime + 1e-6,
      )
  ...
  const entries = skillRows.map(row => {
    ...
    if (row.id === startAnchor.rowId) {
      if (startAnchor.actionIndex !== undefined) {
        const hitNumber = row.actions
          .slice(0, startAnchor.actionIndex + 1)
          .filter(action => action.type === "damage").length
        if (hitNumber > 0) return `${name} (start at hit ${hitNumber})`
      }
      return `${name} (start)`
    }
    if (breakRows.has(row.id)) return `${name} (break)`
    if (row.startTime < anchorTime - 1e-6)
      return `${name} at ${formatHalfSecond(Math.round((anchorTime - row.startTime) * 2) / 2)}`
    return name
  })
