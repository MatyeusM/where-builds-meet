...
const resolved = source
  .replace(/_0x24351d\((-?0x[0-9a-f]+|-?\d+),\s*(-?0x[0-9a-f]+|-?\d+)\)/gi, (match, first, second) =>
    resolve(match, "p", first, second),
  )
  .replace(/_0x4994a7\((-?0x[0-9a-f]+|-?\d+),\s*(-?0x[0-9a-f]+|-?\d+)\)/gi, (match, first, second) =>
    resolve(match, "g", first, second),
  );
