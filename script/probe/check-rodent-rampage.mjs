...
try {
  ...
  const { calculateDamageBreakdown, calculateSimulatedDamageBreakdown } =
    await server.ssrLoadModule("/src/calculations/damage.ts");
  ...
  for (const [distance, coef] of [
    ...
    for (const calculate of [calculateDamageBreakdown, (a, c) => calculateSimulatedDamageBreakdown(a, c, () => 0.5)]) {
      close(
  ...
  console.log(
    "Infernal stage timing, Rodent cadence/lifetime, T6 gating, dynamic damage, and expected/sampled checks passed.",
  );
} finally {
  await server.close();
}
