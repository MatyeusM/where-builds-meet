...
try {
  ...
  const mortal = await load("/data/skill/mortal-rope-dart.json");
  const infernal = await load("/data/skill/infernal-twinblades.json");
  const buffs = await load("/data/buff/bamboocut-wind.json");
  const debuffs = await load("/data/debuff/bamboocut-wind.json");
  const vendetta = await load("/data/innerway/vendetta.json");
  ...
  const reapply = calculateRotationBaseline(
    bundle([cast("BladeboundThreadCancel"), cast("Rodent"), cast("BladeboundThreadCancel"), cast("Rodent"), delay(20)]),
  );
  ...
  console.log("Rodent Hunt recording, reapply/expiry, boundary, cutoff, HP, and sampled-damage checks passed.");
} finally {
  await server.close();
}
