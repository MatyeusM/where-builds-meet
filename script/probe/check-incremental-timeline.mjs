...
try {
  ...
  const { migrateAutomaticCooldownDelays } = await server.ssrLoadModule("/src/rotationEditing.ts");
  const base = {
  ...
  const { withUnresolvedEditorSteps } = await server.ssrLoadModule("/src/editorTimelinePreview.ts");
  ...
  console.log(
    "Incremental scheduling passed: live cooldown resets, no input waits, cast/Delay/Battle End cutoffs and migration anchors.",
  );
} finally {
  await server.close();
}
