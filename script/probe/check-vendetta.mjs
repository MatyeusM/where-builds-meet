...
try {
  ...
  const damageRun = (withToken, t6 = false, setupEffects = []) =>
    calculateRotationBaseline({
      timeline: {
        ...
        innerWayConditions: [],
        innerWayRules: t6
          ? vendetta.effect.VendettaT6.effect.map((effect) => ({ effect: {}, ...effect, source: "Vendetta", tier: 6 }))
          : [],
        setupEffects,
      },
  ...
  const mortalTalents = await load("/data/martial-art/mortal-rope-dart.json");
  ...
  console.log(
    "Vendetta cumulative tiers, extended Rodent triggers, exact expiry, refresh, and expected/sampled checks passed.",
  );
} finally {
  await server.close();
}
