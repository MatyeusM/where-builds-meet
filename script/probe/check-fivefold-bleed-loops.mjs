...
try {
  ...
  const leaves = verifyProbabilityHistories(input) + verifyProbabilityHistories(inputFor([0, 4, 5, 6], 12));
  console.log(
    `T6 guaranteed/optional stacks, damage-before-application, loop cutoff, dummy timing, and ${leaves} concrete probability histories verified.`,
  );
} finally {
  await server.close();
}
