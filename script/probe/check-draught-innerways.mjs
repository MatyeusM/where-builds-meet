...
try {
  ...
  const { resolveAttunementStats } = await server.ssrLoadModule("/src/calculations/attunementStats.ts");
  const base = {
    ...emptyStats,
    minPhys: 100,
    maxPhys: 200,
  ...
  close(attunement.calculation.formlessPenetration + penetration, 20, "The raw contribution must not be counted twice");
  console.log("Draught tier progression, damage/healing channels, and attunement override checks passed.");
} finally {
  await server.close();
}
