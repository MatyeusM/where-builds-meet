...
try {
  const { calculateRotationBaseline, calculateSimulatedRotationRun, calculateRotationComparisons } =
    await server.ssrLoadModule("/src/calculations/rotationCalculator.ts");
  const { emptyStats } = await server.ssrLoadModule("/src/data/statDefinitions.ts");
  const mystic = (await server.ssrLoadModule("/data/skill/mystic.json")).default;
  const mysticBuffs = (await server.ssrLoadModule("/data/buff/mystic.json")).default;
  const general = (await server.ssrLoadModule("/data/buff/general.json")).default;
  const kite = (await server.ssrLoadModule("/data/buff/bamboocut-kite.json")).default;
  const sets = (await server.ssrLoadModule("/data/gear-set.json")).default;
  const stats = {
    ...emptyStats,
    minPhys: 100,
    maxPhys: 100,
  ...
  observed(expiry).forEach((value, index) =>
    near(value, [2880, 2880, 2100][index], "Attack buffs snapshot and stay frozen after expiration; recast refreshes"),
  );
  ...
  for (const [steps, blades] of [
    ...
    sampledFixture.timeline.effectDefinitions = {
      ...
      Attack: {
        ...boost,
        effect: [{ effect: { physicalAttackBonus: 0.5, silkbindAttackBonus: 0.2 } }],
      },
    };
  ...
  console.log(
    "WTS cast snapshots: exact cast state, food, buff expiry, recasting, nearby-skill isolation, Hawkwing/Etherwrath feedback, expected and sampled healing verified.",
  );
} finally {
  await server.close();
}
