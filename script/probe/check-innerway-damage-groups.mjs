...
try {
  ...
  const { buildRotationTimeline, mergeCalculatedTimelineState } = await server.ssrLoadModule(
    "/src/calculations/rotationTimeline.ts",
  );
  const { buildTimelineDisplayEntries } = await server.ssrLoadModule("/src/rotationDisplay.ts");
  ...
  const buffs = await load("/data/buff/general.json");
  ...
  const strip = (definitions) =>
    Object.fromEntries(Object.entries(definitions).map(([id, { damageGroup, ...definition }]) => [id, definition]));
  ...
  console.log(
    "Inner Way ownership, unchanged damage/timing, grouped totals, zero-proc headers, expansion isolation, and sampled ownership verified.",
  );
} finally {
  await server.close();
}
