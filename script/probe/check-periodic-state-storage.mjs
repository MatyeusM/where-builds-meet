...
const referenceSource = execFileSync("git", ["show", "78537e2:src/calculations/outcomeTriggeredBuffs.ts"], {
  encoding: "utf8",
});
const servers = [];
try {
  const makeServer = async (reference) => {
        ...
        ? [
            {
              ...
              enforce: "pre",
              transform(code, id) {
                if (id.replaceAll("\\", "/").endsWith("/src/calculations/outcomeTriggeredBuffs.ts"))
  ...
  const referenceServer = await makeServer(true);
  const { ExpectedPeriodicTracker: Current } = await server.ssrLoadModule("/src/calculations/outcomeTriggeredBuffs.ts");
  const { ExpectedPeriodicTracker: Reference } = await referenceServer.ssrLoadModule(
    "/src/calculations/outcomeTriggeredBuffs.ts",
  );
  const { periodicStateListFactory, mergePeriodicLists, mergeTinyPeriodicEntries } = await server.ssrLoadModule(
    "/src/calculations/periodicStateLists.ts",
  );
  ...
  console.log(
    "Both storage backends match the committed probability model across randomized refreshes, owners, gains, expirations, burst follow-ups and exact/shared cadences; slot reuse and sorted operations pass.",
  );
} finally {
