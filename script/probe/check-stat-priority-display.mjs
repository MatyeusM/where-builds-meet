...
try {
  const { statPriorityDisplayRows, nextStatPriorityMode } = await server.ssrLoadModule("/src/statPriorityDisplay.ts");
  const { maxGearRoll } = await server.ssrLoadModule("/src/gear.ts");
  ...
  console.log(
    "Stats priority display preserves Max results, scales relayed rolls and DPS/HPS predictions, interleaves both modes, handles ties/negative/empty values, and leaves cached inputs untouched.",
  );
} finally {
  await server.close();
}
