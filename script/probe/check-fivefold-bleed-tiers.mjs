...
try {
  ...
  const { buildRotationTimeline } = await load("/src/calculations/rotationTimeline.ts");
  ...
  const { calculateStatsWithEffects } = await load("/src/calculations/statEffects.ts");
  ...
  console.log(
    "Fivefold Bleed T1/T2 scaling and T3 expiration, refresh, removal, duplicate-schedule, and exhaustive probability checks passed.",
  );
} finally {
  await server.close();
}
