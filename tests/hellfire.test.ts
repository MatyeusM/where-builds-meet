...
describe("Hellfire", () => {
  it.each([false, true])(
    ...
    (sampled) => {
      ...
      expect(rows.filter((row) => row.step.event === "Hellfire").map((row) => row.effectiveCastTime)).toEqual([
        0, 0, 0, 0,
      ]);
    },
  );
  ...
  it.each([
    ...
    expect(rows.find((row) => row.step.type === "skill" && row.step.skill === "End")!.resources.Hellfire).toBeCloseTo(
      10,
    );
  });
