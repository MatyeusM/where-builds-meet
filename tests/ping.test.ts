...
describe("ping scheduling", () => {
  ...
  it("waits for cooldown then pays ping once, including live resets during dispatch", () => {
    ...
    expect(rows.find((row) => row.step.type === "event" && row.step.event === "Delay")?.effectiveCastTime).toBeCloseTo(
      2,
    );
...
it.each(["Deflect", "DeflectSuccessful"])("%s ignores ping without an incoming attack", (skill) => {
  ...
  expect(rows.map((row) => row.startTime)).toEqual(zeroPingRows.map((row) => row.startTime));
  expect(rows[0].startTime).toBe(0);
  expect(rows[1].startTime).toBeCloseTo(rows[0].effectiveCastTime);
});
