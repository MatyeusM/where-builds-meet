...
for (const sampled of [false, true]) {
  ...
  describe(`Enhanced Rodent Rampage (sampled=${sampled})`, () => {
    ...
    it.each([
      ...
      if (skill === "InfernalFlamelashLight5Cancel") {
        expect(times).toHaveLength(4); // One coordinated, two T6, one automatic.
        expect(times.filter((time) => Math.abs(time - appliedAt - 0.842) < 1e-6)).toHaveLength(3);
      } else {
        expect(times[0]).toBeCloseTo(appliedAt + 0.5);
