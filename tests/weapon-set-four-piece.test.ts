  ...
  it("checks conditional set behavior and simulation", async () => {
    ...
    for (const roll of [undefined, () => 0.5]) {
      ...
      for (const [tier, skill] of [
        ...
        assert.ok(
          !rows.find(row => row.step.skill === "Probe").actionStates[0].buffs.some(buff => buff.name === "Cleftpeak"),
      ...
      assert.ok(
        !expired.find(row => row.step.skill === "Probe").actionStates[0].buffs.some(buff => buff.name === "Cleftpeak"),
