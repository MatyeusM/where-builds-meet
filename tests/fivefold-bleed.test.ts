...
describe("fivefold-bleed", () => {
  it("Fivefold Bleed chance, atomic threshold bursts, fresh cadence, generic application rules, exhaustive expected-state and simulation checks passed", async () => {
    ...
    assert.equal(
      ticks(buildRotationTimeline(inputFor([0, 1]), () => 0.1)).length,
      0,
      "Roll at 10% boundary must fail.",
    );
    assert.equal(
    ...
    assert.equal(
      ...
      2,
      "Ten successful simultaneous applications must produce exactly two bursts.",
    );
