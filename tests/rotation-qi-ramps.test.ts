...
// Ported from script/probe/check-rotation-qi-ramps.mjs.
describe("rotation-qi-ramps", () => {
  // STALE: fails identically on main via script/probe/check-rotation-qi-ramps.mjs
  // (Mixed Dummy IV Qi ramp timing drift (11.317s vs ~12.109s)). Kept for future repair instead of deleting the coverage.
  it.skip("rotation-qi-ramps checks", async () => {
    ...
    for (const rotationPath of rotationPaths) {
      ...
      if (process.env.PROBE_SUGGEST !== undefined) {
      ...
      for (const ratio of rampRatios) {
        ...
        for (let index = 0; index < zeroRows.length; index += 1) {
          ...
          expect(rampTime < zeroTime, `${rotation.name} Qi ramp ${index + 1} must precede depletion.`).toBeTruthy();
          expect(
