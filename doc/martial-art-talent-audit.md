...
| Stormbreaker Spear   | Higher of Body/Power to Max HP needs a max-of-two-stat expression (10 HP per point, cap 2800). Battle Will cap increases need base resource/cap data and talent-dependent maximums. Three-second incoming Physical damage reduction needs typed incoming damage and reduction scaling. |
| Thundercry Blade     | Higher of Body/Power to Max Physical requires a max-of-two-stat expression. Removed the incorrect Power-only approximation instead of retaining a misleading value.                                                                                                                    |
| Phalanxbane Blade    | Blade Momentum maximum and Anxi Soldier refunds need resource data; “four times per second” is not assumed equivalent to a 0.25-second cooldown.                                                                                                                                       |
| Infernal Twinblades  | Dodge success window and breath-hold are unmodeled. Addled Mind and automatic Flamelash entry/exit need skill data. Earlier user-confirmed charge/reset semantics are retained.                                                                                                        |
...
| Soulshade Umbrella   | Party-wide copies and Tenacity behavior/per-recipient 60-second lockout are not modeled. The supported self damage buff follows Floating Grace's existing application timing.                                                                                                          |
| Everspring Umbrella  | Perfect Catch input timing, returning-flight timing, and Tenacity damage are missing. Falling Blossoms/Fragrant Song's full transition and one-use payload await Scarlet Spin's skill data; no partial flight/critical state is guessed.                                               |
...
| Unfettered Rope Dart | Piercing Dart/Soul Sweep skill data and token resource units are missing: the text mixes two Tokens with a ten-Token refund. Combo-count and 0.5-second cooldown reduction are deferred; `clearCD` would incorrectly reset the whole cooldown.                                         |
...
| Snowparting Blade    | Blade Momentum needs resource setup/generation data before its condition activates in normal rotations. Tenacity interrupt, stagger, boss/mode distinctions, and General's Bane: Stab's 2/6-second Dread extension lack sufficient classification/state data.                          |
...
has its stated 20-second refreshable lifetime. `Binge` and `BladeMomentum` reuse
numeric resources; neither receives an invented global starting value or cap.
