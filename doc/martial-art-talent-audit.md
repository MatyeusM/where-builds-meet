...
| Everspring Umbrella  | Perfect Catch input timing, returning-flight timing, and Tenacity damage are missing. Falling Blossoms now transitions into Fragrant Song after three explicit successful catches. One-use damage/consumption is implemented for explicit throws; Tenacity remains deferred. Fragrant Song's faster flight and accelerated-flight guaranteed catch are intentionally ignored. |
...
| Unfettered Rope Dart | Piercing Dart/Soul Sweep hit timing and token resource units are unresolved: the text mixes two Tokens with a ten-Token refund. Charged Combo uses `clearCD.seconds` for 0.5 seconds per Piercing Dart damage hit with a 0.5-second trigger cooldown; activation awaits the damage events.                                                                                    |
